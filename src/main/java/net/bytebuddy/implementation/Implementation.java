        ...
        class Default extends ExtractableView.AbstractBase {
            ...
            public void drain(TypeInitializer.Drain drain,
                ...
                for (Map.Entry<FieldCacheEntry, FieldDescription.InDefinedShape> entry : registeredFieldCacheEntries.entrySet()) {
                    //TODO ignore duplicated field
                    try {
                        Field instrumentedTypeField = ReflectionUtils.findField(drain.getClass(), "instrumentedType");
                        if (instrumentedTypeField != null) {
                            instrumentedTypeField.setAccessible(true);
                            InstrumentedType _instrumentedType = (InstrumentedType) instrumentedTypeField.get(drain);
                            String fieldName = entry.getValue().getName();
                            FieldList<FieldDescription.InDefinedShape> declaredFields = _instrumentedType.getDeclaredFields();
                            if (declaredFields.stream().filter(f -> f.getName().equals(fieldName)).findAny().isPresent()){
                                continue;
                            }
                        }
                    } catch (Exception e) {
                        e.printStackTrace();
                    } // ignore duplicated field end
